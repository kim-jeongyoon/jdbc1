...
public class JDBCExample {
	...
	public static void main(String[] args) {
		...
		try {
			...
			// 4. Statement 객체생성 -> connection 객체를 통해서 생성
			...
			//execyteQuery : SELECT문 수행 메서드, ResultSet 반환
			
			
			
			
			
			// 3단계: SQL을 수행해서 반환 받은 결과(Result Set)를 
			//			한행씩 접근해서 컬럼 값 얻어오기
			
			while(rs.next()) {
				// rs.next() : rs가 참조하는 ResultSet객체의 첫번째 컬럼부터 순서대로
				//				한 행씩 이동하며 다음 행이 있을 경우 true 반환/ 없으면 false 반환
